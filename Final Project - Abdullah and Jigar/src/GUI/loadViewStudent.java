...
public class loadViewStudent {
	 ...
	 * To table model.
	 *
	 * @return the table model that displays all the student in the system
	 *         (Student HashMap)
	 */
	 ...
	 * @return the table model that returns the student associated to the ID
	 *         being searched for. If not found displays a 'not found' message.
	 */
	private static TableModel binarySearch(StudentHashMap studentMap, int keyId, int keyStart, int high) {
		...
		if (keyId == mid) {
			...
			if (studentHashMap.containsKey(keyId)) {
				String fname = studentMap.getFirstName(keyId);
				String lname = studentMap.getLastName(keyId);
				String major = studentMap.getMajor(keyId);
				String year = studentMap.getYear(keyId);
				String GPA = studentMap.getGPA(keyId);
