...
/**
 * The Class loadRemoveStudent defines the layout that remove students from the database (Student HashMap)
 * 
...
public class loadRemoveStudent {
	 ...
	 * 
	 * @param studentMap the StudentHashMap
	 */
		...
		btnRemoveStudent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//if input is not empty then it proceeds to remove student
					...
					//checks if input is an Integer. If its integer then removes the ID from the Database (Student HashMAp)
					try {
						int id = Integer.parseInt(textField.getText().trim());
						
					
							if (studentHashMap.containsKey(id)) {
								JOptionPane.showMessageDialog(frame, "Removed: "+ "(" +id + ")" + studentHashMap.getFirstName(id) + " " + studentHashMap.getLastName(id));
